...
# Your name: Adam Casenas 
# Your student id: 48967091
# Your email: acasenas@umich.edu
...
def make_players_table(data, cur, conn):
    ...
    for player in data['squad']:
        ...
        players.append((idNum, name, position, birthyear, nationality))
    ...
    for i in range(len(players)):
        positionS = players[i][2]
        cur.execute("SELECT id FROM Positions Where position = ?", (positionS,))
        positionNum = cur.fetchone()[0]
        cur.execute("INSERT OR IGNORE INTO Players (id, name, position_id, birthyear, nationality) VALUES (?,?,?,?,?)",(players[i][0], players[i][1], positionNum, players[i][3], players[i][4]))
    conn.commit()
...
def birthyear_nationality_search(age, country, cur, conn):
    year = 2023 - age
    cur.execute("SELECT name, nationality, birthyear FROM Players Where nationality = ? AND birthyear <= ?", (country, year))
    miniLst = cur.fetchall()
    return miniLst
